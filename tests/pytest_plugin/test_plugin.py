"""Tests for the evaldata pytest plugin's `case` fixture, exercised via `pytester`."""
...
from evaldata.pytest_plugin import plugin
...
def test_case_fixture_injects_the_decorated_case(pytester: pytest.Pytester) -> None:
    pytester.makepyfile(
        """
        from evaldata import eval_case
...
def test_run_summary_printed_at_end_of_session(pytester: pytest.Pytester) -> None:
    ...
    result = pytester.runpytest()
    result.assert_outcomes(passed=1)
    result.stdout.fnmatch_lines(["*evaldata summary*", "*test_eval*PASS*", "*1 passed, 0 failed*"])
...
def test_json_artifact_written_when_flag_set(pytester: pytest.Pytester) -> None:
    ...
    result = pytester.runpytest(f"--evaldata-json={artifact}")
    result.assert_outcomes(passed=1)
...
def test_xdist_aggregates_worker_results_into_summary_and_json(pytester: pytest.Pytester) -> None:
    ...
    artifact = pytester.path / "results.json"
    result = pytester.runpytest_subprocess("-n", "2", f"--evaldata-json={artifact}")
    result.assert_outcomes(passed=1)
    result.stdout.fnmatch_lines(["*evaldata summary*", "*test_eval*PASS*", "*1 passed, 0 failed*"])
    payload = json.loads(artifact.read_text())
    assert payload["passed"] == 1
